...
label=f"Estimated: {task[0]}")
for i, (task, act_start, act_end) in enumerate(zip(tasks, actual_start_dates, actual_end_dates)):
ax.barh(i + bar_height/2, (act_end - act_start).days, left=act_start, height=bar_height, color='orange',
label=f"Actual: {task[0]}")
...
ax.legend(handles[:len(unique_labels)], unique_labels, loc="upper left", bbox_to_anchor=(1, 1))
plt.tight_layout()
plt.show()